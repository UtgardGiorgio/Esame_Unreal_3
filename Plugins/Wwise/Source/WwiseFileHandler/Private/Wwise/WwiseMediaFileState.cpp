...
void FWwiseStreamedMediaFileState::CloseFile(FCloseFileCallback&& InCallback)
{
	...
	if (UNLIKELY(!MediaManager))
	{
		UE_LOG(LogWwiseFileHandler, Log, TEXT("FWwiseStreamedMediaFileState::CloseFile %" PRIu32 " (%s): Failed closing prefetch without the MediaManager. Leaking."), MediaId, *DebugName.ToString());
		PackagedFile.DeallocateMemory(pMediaMemory, uMediaSize, true, STAT_WwiseMemoryMediaPrefetch_FName, STAT_WwiseMemoryMediaPrefetchDevice_FName);
		pMediaMemory = nullptr;
		uMediaSize = 0;
		return CloseFileDone(MoveTemp(InCallback));
	...
	if (MediaManager->IsProcessingTerm())
	{
		return CloseFilePrefetchInSoundEngine(MoveTemp(InCallback));
	}
	...
	{
		CloseFilePrefetchInSoundEngine(MoveTemp(InCallback));
	});
}

void FWwiseStreamedMediaFileState::CloseFilePrefetchInSoundEngine(FCloseFileCallback&& InCallback)
{
	auto* SoundEngine = IWwiseSoundEngineAPI::Get();
	if (UNLIKELY(!SoundEngine))
	{
