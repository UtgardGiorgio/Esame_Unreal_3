...
void FWwiseResourceLoaderImpl::UpdateLanguage(FWwiseSetLanguagePromise&& Promise, const FWwiseLanguageCookedData& InLanguage, EWwiseReloadLanguage InReloadLanguage)
{
	...
	Future.Next([WeakThis=AsWeak(), OldLanguage = MoveTemp(OldLanguage), NewLanguage = MoveTemp(NewLanguage), Promise = MoveTemp(Promise)](int) mutable
	{
		...
		SharedResourceLoader->ExecutionQueue->Async(WWISERESOURCELOADER_ASYNC_NAME("FWwiseResourceLoaderImpl::SetLanguageAsync Unloading"), [WeakThis=MoveTemp(WeakThis), OldLanguage = MoveTemp(OldLanguage), NewLanguage = MoveTemp(NewLanguage), Promise = MoveTemp(Promise)]() mutable
		{
			...
			{
				if (LoadedSoundBank.LanguageRef == FWwiseLanguageCookedData::Sfx)
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: Skipping unloading SoundBank %s with language SFX"),
						*LoadedSoundBank.LocalizedSoundBankCookedData.DebugName.ToString());
					continue;
				}

				if (auto* SoundBank = LoadedSoundBank.LocalizedSoundBankCookedData.SoundBankLanguageMap.Find(LoadedSoundBank.LanguageRef))
				{
				...
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No SoundBank %s with language %s to unload"),
			...
			{
				if (LoadedAuxBus.LanguageRef == FWwiseLanguageCookedData::Sfx)
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: Skipping unloading AuxBus %s with language %s"),
						*LoadedAuxBus.LocalizedAuxBusCookedData.DebugName.ToString(), *LoadedAuxBus.LanguageRef.GetLanguageName().ToString());
					continue;
				}

				if (auto* AuxBus = LoadedAuxBus.LocalizedAuxBusCookedData.AuxBusLanguageMap.Find(LoadedAuxBus.LanguageRef))
				{
				...
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No AuxBus %s with language %s to unload"),
						*LoadedAuxBus.LocalizedAuxBusCookedData.DebugName.ToString(), *LoadedAuxBus.LanguageRef.GetLanguageName().ToString());
				}
			}
			...
			{
				if (LoadedShareSet.LanguageRef == FWwiseLanguageCookedData::Sfx)
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: Skipping unloading ShareSet %s with language %s"),
						*LoadedShareSet.LocalizedShareSetCookedData.DebugName.ToString(), *LoadedShareSet.LanguageRef.GetLanguageName().ToString());
					continue;
				}
				
				if (auto* ShareSet = LoadedShareSet.LocalizedShareSetCookedData.ShareSetLanguageMap.Find(LoadedShareSet.LanguageRef))
				{
				...
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No ShareSet %s with language %s to unload"),
						*LoadedShareSet.LocalizedShareSetCookedData.DebugName.ToString(), *LoadedShareSet.LanguageRef.GetLanguageName().ToString());
				}
			}
			for (auto& LoadedEvent : SharedResourceLoader->LoadedEventList)
			{
				if (LoadedEvent.LanguageRef == FWwiseLanguageCookedData::Sfx)
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: Skipping unloading Event %s with language %s"),
						*LoadedEvent.LocalizedEventCookedData.DebugName.ToString(), *LoadedEvent.LanguageRef.GetLanguageName().ToString());
					continue;
				}

				if (auto* Event = LoadedEvent.LocalizedEventCookedData.EventLanguageMap.Find(LoadedEvent.LanguageRef))
				{
				...
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No Event %s with language %s to unload"),
						*LoadedEvent.LocalizedEventCookedData.DebugName.ToString(), *LoadedEvent.LanguageRef.GetLanguageName().ToString());
				}
			}
				...
				AffectedSoundBanks = MoveTemp(AffectedSoundBanks)]() mutable
			{
				...
				for (auto* LoadedSoundBank : AffectedSoundBanks)
				{
					...
					{
						UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No SoundBank %s with language %s to load"),
							*LoadedSoundBank->LocalizedSoundBankCookedData.DebugName.ToString(), *LoadedSoundBank->LanguageRef.GetLanguageName().ToString());
					}
				}
				...
				for (auto* LoadedAuxBus : AffectedAuxBusses)
				{
					...
					{
						UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No AuxBus %s with language %s to load"),
							*LoadedAuxBus->LocalizedAuxBusCookedData.DebugName.ToString(), *LoadedAuxBus->LanguageRef.GetLanguageName().ToString());
					}
				}
				...
				for (auto* LoadedShareSet : AffectedShareSets)
				{
					...
					{
						UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No ShareSet %s with language %s to load"),
							*LoadedShareSet->LocalizedShareSetCookedData.DebugName.ToString(), *LoadedShareSet->LanguageRef.GetLanguageName().ToString());
					}
				}
				...
				for (auto& LoadedEvent : AffectedEvents)
				{
					...
					{
						UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("SetLanguage: No Event %s with language %s to load"),
							*LoadedEvent->LocalizedEventCookedData.DebugName.ToString(), *LoadedEvent->LanguageRef.GetLanguageName().ToString());
					}
				}
...
void FWwiseResourceLoaderImpl::LoadEventResources(FWwiseResourceLoadPromise&& Promise, FWwiseLoadedEventInfo::FLoadedData& LoadedData, const FWwiseEventCookedData& InCookedData)
{
	...
	if (InCookedData.RequiredGroupValueSet.Num() > 0 || InCookedData.SwitchContainerLeaves.Num() > 0)
	{
		...
		ExecutionQueue->Async(TEXT("FWwiseResourceLoaderImpl::LoadEventResources"), [This = AsShared(), SwitchContainerPromise = MoveTemp(SwitchContainerPromise), &LoadedData, &InCookedData]() mutable
		{
			auto& Impl = static_cast<FWwiseResourceLoaderImpl&>(This.Get());
			Impl.LoadEventSwitchContainerResources(MoveTemp(SwitchContainerPromise), LoadedData, InCookedData);
		});
...
void FWwiseResourceLoaderImpl::LoadEventSwitchContainerResources(FWwiseResourceLoadPromise&& Promise, FWwiseLoadedEventInfo::FLoadedData& LoadedData, const FWwiseEventCookedData& InCookedData)
{
	...
	for (const auto& GroupValue : InCookedData.RequiredGroupValueSet)
	{
		...
		GroupValueResourceFuture.Next([WeakThis=AsWeak(), &LoadedRequiredGroupValues, &InCookedData, &GroupValue, GroupValuePromise = MoveTemp(GroupValuePromise), LoadedNode](bool bResult) mutable
		{
	...
	for (const auto& SwitchContainerLeaf : InCookedData.SwitchContainerLeaves)
	{
		...
		for (const auto& GroupValue : SwitchContainerLeaf.GroupValueSet)
		{
			...
			if (Info.ResourcesAreLoaded())
			{
				...
				if (UNLIKELY(bIsAlreadyCreated))
				{
					UE_LOG(LogWwiseResourceLoader, Error, TEXT("Loading already created Switch Container Leaf LoadedGroupValueCount %d @ %p for %s"),
						(int)UsageCount->LoadedGroupValues.Num(), &UsageCount->LoadedData, *UsageCount->Key.GetDebugString());
					SCOPED_WWISERESOURCELOADER_EVENT_3(TEXT("FWwiseResourceLoaderImpl::LoadEventSwitchContainerResources AudioNode.SwitchContainer AlreadyLoaded Done"));
					return SwitchContainerLeafPromise.EmplaceValue();
				}
			...
			{
				SCOPED_WWISERESOURCELOADER_EVENT_3(TEXT("FWwiseResourceLoaderImpl::LoadEventSwitchContainerResources AudioNode.SwitchContainer !ShouldBeLoaded.Done"));
...
void FWwiseResourceLoaderImpl::UnloadEventResources(FWwiseResourceUnloadPromise&& Promise, FWwiseLoadedEventInfo::FLoadedData& LoadedData, const FWwiseEventCookedData& InCookedData)
{
	...
	if (LoadedData.bLoadedSwitchContainerLeaves || LoadedData.LoadedRequiredGroupValues.Num() > 0)
	{
		...
		ExecutionQueue->Async(TEXT("FWwiseResourceLoaderImpl::UnloadEventResources"), [This = AsShared(), SwitchContainerLeavesPromise = MoveTemp(SwitchContainerLeavesPromise), &LoadedData, &InCookedData]() mutable
		{
			auto& Impl = static_cast<FWwiseResourceLoaderImpl&>(This.Get());
			Impl.UnloadEventSwitchContainerResources(MoveTemp(SwitchContainerLeavesPromise), LoadedData, InCookedData);
		});
	}
...
void FWwiseResourceLoaderImpl::UnloadEventSwitchContainerResources(FWwiseResourceUnloadPromise&& Promise, FWwiseLoadedEventInfo::FLoadedData& LoadedData, const FWwiseEventCookedData& InCookedData)
{
	...
	if (bLoadedSwitchContainerLeaves) for (const auto& SwitchContainerLeaf : InCookedData.SwitchContainerLeaves)
	{
		...
		FutureArray.Add(SwitchContainerLeavesPromise.GetFuture());
			...
			UnloadLeafResourcesFuture.Next([WeakThis = AsWeak(), SwitchContainerLeavesPromise = MoveTemp(SwitchContainerLeavesPromise), UsageCount](int) mutable
			{
				auto SharedResourceLoader = StaticCastSharedPtr<FWwiseResourceLoaderImpl>(WeakThis.Pin());
				if (!SharedResourceLoader.IsValid())
				{
...
void FWwiseResourceLoaderImpl::UnloadSwitchContainerLeafResources(FWwiseResourceUnloadPromise&& Promise, TSharedRef<FWwiseSwitchContainerLeafGroupValueUsageCount, ESPMode::ThreadSafe> UsageCount)
{
	...
	if (LoadedData.IsProcessing)
	{
		// If this Switch Container leaf keeps rescheduling for longer than the threshold, a
		// deferred resource unload is likely not completing (e.g. media stuck AK_ResourceInUse
		// because RenderAudio is not ticking).
		{
			const double NowSeconds = FPlatformTime::Seconds();
			if (LoadedData.UnloadRescheduleStartSeconds == 0.0)
			{
				LoadedData.UnloadRescheduleStartSeconds = NowSeconds;
				LoadedData.bUnloadStallReported = false;
			}
			else if (!LoadedData.bUnloadStallReported && NowSeconds - LoadedData.UnloadRescheduleStartSeconds > 5.0)
			{
				LoadedData.bUnloadStallReported = true;
				UE_LOG(LogWwiseResourceLoader, Error, TEXT("FWwiseResourceLoaderImpl::UnloadSwitchContainerLeafResources UsageCount[%p]: %s has been rescheduling its unload for %.1f s while still processing. A deferred unload is likely stalled."),
					&UsageCount.Get(), *CookedData.GetDebugString(), NowSeconds - LoadedData.UnloadRescheduleStartSeconds);
			}
		}
	...
	// Not rescheduling: the unload is proceeding, so clear any stall-tracking state.
	LoadedData.UnloadRescheduleStartSeconds = 0.0;
	LoadedData.bUnloadStallReported = false;
	...
	for (const auto& ExternalSource : InExternalSources)
	{
		...
		LoadExternalSourceFile(ExternalSource, [WeakThis=AsWeak(), &ExternalSource, &LoadedExternalSources, InType, InDebugName, InShortId, Promise = MoveTemp(Promise)](bool bInResult) mutable
		{
			...
			else
			{
				...
				{
					UE_LOG(LogWwiseResourceLoader, VeryVerbose, TEXT("AddLoadExternalSourceFutures: Adding External Source %s (%" PRIu32 ") to %s %s (%" PRIu32 ")"),
						*ExternalSource.DebugName.ToString(), (uint32)ExternalSource.Cookie,
						InType, *InDebugName, (uint32)InShortId);
	...
	const TCHAR* InType, const FString& InDebugName, uint32 InShortId) const
{
	...
	UE_LOG(LogWwiseResourceLoader, Verbose, TEXT("AddUnloadMediaFutures: Unloading all %d Media from %s %s (%" PRIu32 ")"),
		(int)ToUnload.Num(),
		InType, *InDebugName, (uint32)InShortId);
                                                         ...
                                                         const TCHAR* InType, const FString& InDebugName, uint32 InShortId) const
{
	...
	UE_LOG(LogWwiseResourceLoader, Verbose, TEXT("AddUnloadSoundBankFutures: Unloading all %d SoundBanks from %s %s (%" PRIu32 ")"),
		(int)ToUnload.Num(),
		InType, *InDebugName, (uint32)InShortId);
